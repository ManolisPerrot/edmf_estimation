...
BuildNewEmulators <- function(tData, HowManyEmulators, 
  ...
  Prior_Em=Emulators
  Emulators <- try(mogp_emulator$fit_GP_MAP(Prior_Em, n_tries=1, ftol=1e-06, theta0 = Starts, maxiter=5000))
  if(inherits(Emulators, "try-error")){
        Emulators <- try(mogp_emulator$fit_GP_MAP(Prior_Em, n_tries=4, ftol=1e-06, theta0 = Starts, maxiter=5000))}
   if(inherits(Emulators, "try-error")){
         Emulators <- try(mogp_emulator$fit_GP_MAP(Prior_Em, n_tries=40, ftol=1e-06, theta0 = Starts, maxiter=5000))}
   if(inherits(Emulators, "try-error")){
         Emulators <- try(mogp_emulator$fit_GP_MAP(Prior_Em, n_tries=400, ftol=1e-06, theta0 = Starts, maxiter=5000))}
  if(inherits(Emulators, "try-error")){
	print(Emulators)
  	print("4 tries failed to find a good optimisation, go to BuildEmulator.R and change the value of n_tries ")
  }
...
LOO.plot <- function(Emulators, which.emulator=1, ParamNames,
                     ...
                     ObsErr=NULL, ParRange=FALSE, ObsRange=FALSE) {
  ...
  #' @param ObsRange. Boolean to indicate if the plot window should have y ranges that include
  #' obs uncertainty (defaults to FALSE to facilitate emulator diagnostics)
  #' @param ParRange. Boolean to indicate if the plot window should have x ranges that include
  #' parameter initial range (defaults to FALSE to facilitate emulator diagnostics)
  #' 
  #' @return a data frame with three columns, with first column corresponding to posterior mean, 
  #' and second and third columns corresponding to the minus and plus two standard deviations.
  ...
  if(ObsRange){
    fit.loo <- rbind(fit.loo, c(Obs, Obs-2.2*ObsErr,Obs+2.2*ObsErr))
  }
  ...
  for(i in 1:p) {
    ...
    try(aplot <- ValidPlotNew(fit = fit.loo, x = Design[,i], y=Emulators$mogp$emulators[[which.emulator]]$targets, 
                              ...
                              xlim=xlim, xlab=param.names[i], log=tlogs[i], 
                              ylab=Emulators$fitting.elements$lm.object[[which.emulator]]$ResponseString), silent=TRUE)
    if(!inherits(aplot, "try-error") & !is.null(Obs)){
      abline(h=Obs, lty=2, lwd=2, col=2)
      if(is.null(ObsErr))
        warning("The observations do not have 0 error. Please add ObsErr else the plot will be misleading")
      else{
        abline(h=Obs+2*ObsErr, lwd=3, col=2, lty=1)
        abline(h=Obs-2*ObsErr, lwd=3, col=2, lty=1)
        #legend("bottomright", legend = c("Obs", "+/- 2 tolerance"), lwd = c(2,4), lty=(1,2), col = c(2,2))
      }      }
  }
  return(fit.loo)
}
