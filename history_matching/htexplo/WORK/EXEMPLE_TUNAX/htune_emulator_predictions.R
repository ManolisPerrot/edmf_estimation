...
file <- paste0("WAVE", WAVEN, "/Plots_Predictions.pdf")
pdf(file = file, width = 12, height = 6)

cat("==== Computing Emulator Predictions for wave ", WAVEN, " =====\n")
...
for (k_wave in 1:WAVEN) {
  ...
  n_metrics <- Emulator$mogp$n_emulators

  for (k_metric in 1:n_metrics) {
    ...
    for (k_point in 1:sample_size) {
      max_implausibilite[k_point] <- max(
        max_implausibilite[k_point],
        Implausibilite[k_point]
      )
    }
    ...
    table_to_write <- cbind(table_to_write,
                            Emul_expectation,
                            Emul_variance)

    names_metric_table <- c(
      names_metric_table,
      paste0("E_", MetricNames[k_metric], "_WAVE", k_wave),
      paste0("V_", MetricNames[k_metric], "_WAVE", k_wave)
    )
    ...
    for (k_param in 1:nparam_to_plot) {
      ...
      plot(vec_param,
           ...
           ylab = MetricNames[k_metric],
           col  = colors[k_wave],
           main = paste("Wave", k_wave, "- Metric", MetricNames[k_metric])
      )
  ...
  wave_when_ruled_out[
    wave_when_ruled_out == 0 &
      max_implausibilite > cutoff_vec[k_wave]
  ] <- k_wave
}
...
# UFILE <- paste0("Predictions_Wave", WAVEN, ".asc")

# table_to_write <- cbind(table_to_write, wave_when_ruled_out)

# colnames(table_to_write) <- c(
#   ParNames[1:nparam],
#   names_metric_table,
#   "nwave_ruled_out"
# )
...
# writeLines(paste("#", nparam, nmetrique), UFILE)
...
# write header line
write(paste("#", nparam, nmetrique), file = UFILE)

# write table (IMPORTANT: no append)
write.table(table_to_write,
            file = UFILE,
            row.names = FALSE,
            col.names = TRUE,
